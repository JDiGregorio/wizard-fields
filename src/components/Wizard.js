...
export const useWizardContext = () => {
    ...
    if (!context) {
        throw new Error(`Un componente compuesto no puede ser renderizado fuera del padre`)
    }
...
const Wizard = ({ children, reducer = defaultReducer, initialState }) => {
    const [{ activePageIndex, steps }, dispatch] = React.useReducer(combineReducers(wizardReducer, reducer), {
            ...defaultInitialState,
            ...initialState
        }
    )
